...
# Diccionario con Nombres Completos para que no haya dudas
ACTIVOS_DETALLADOS = {
...
async def responder_tendencias(update, context):
    ...
    if "tendencias actuales" in user_text:
        msg_inicial = await update.message.reply_text("⏳ *Consultando mercados... Por favor espera.*", parse_mode="Markdown")
        ...
        async with websockets.connect(uri) as ws:
            for nombre_claro, api_code in ACTIVOS_DETALLADOS.items():
                ...
                reporte += linea + "\n\n"
                await asyncio.sleep(0.1) # Respetar límites de la API
...
def main():
    print("🤖 Bot de Consultas Live Iniciado...")
